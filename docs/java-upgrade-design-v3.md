# Java バージョンアップ支援ツール 開発方針書 v3

本書は単体で読める自己完結の方針書である(旧版 v1/v2 の参照は不要)。v2 までの設計レビューおよび 2026-07-07〜08 のヒアリングでの決定をすべて反映した。主要な設計判断とその理由は付録 A にまとめる。

## 1. 目的と基本原則

### 1.1 目的

古い Java LTS バージョン(代表例: Java 7)で稼働する既存システムのソースコードを、生成 AI(LLM)を活用して目標の LTS バージョン(標準: Java 21)まで段階的にバージョンアップする**汎用ツール**を開発する。公開されている各バージョンの互換性情報(リリースノート、JEP、移行ガイド)を根拠として、必要最小限の修正を適用する。

### 1.2 基本原則

| # | 原則 | 内容 |
|---|------|------|
| 1 | 振る舞い保証優先 | アップグレード開始前に開始バージョンの JVM 上で Characterization Test(Golden Master 方式)を作成・凍結し、全移行完了まで**同一のテストバイトコード**が通り続けることを合格基準とする。保証範囲は「カバーされた実行パス」であり、保証範囲率を第一級の報告値とする(7 章) |
| 2 | リファクタリング禁止 | 変更は「新バージョンでコンパイル・動作するために必要な修正」のみ。構文モダン化は行わない。機械的 AST 判定で強制する(4.2.3 節)。例外: 人間による手動修正は validate_diff の対象外(4.3.4 節) |
...
| 7 | 凍結テストの唯一の例外 | 凍結テストの変更は原則禁止。唯一の例外は、人間が「非本質挙動の固定だった」と承認した場合の**再ベースライン**(4.1.5 節)。自動再ベースラインは禁止 |

## 2. スコープと前提

- **汎用ツール**である。ただしビルドシステムは **Gradle のみ**対応する
- バージョン経路は LTS のみを経由する `Java 7 → 8 → 11 → 17 → 21` を全体経路とし、**開始・終了バージョンはその部分列として設定可能**(例: 8→17)。以下、開始バージョンを N₀、目標バージョンを N_t と書く
- マルチモジュール(マルチプロジェクト)構成は「**全サブプロジェクト一斉**」で扱う(サブプロジェクトごとの段階進行はしない)。初期実装のマイルストーンはシングルモジュールから刻む
- 対象プロジェクトの **Gradle 定義(build.gradle / settings.gradle)はツールが編集しない**。ビルド条件の差し替えはすべて init script オーバーレイで行う(5.3 節)
- 移行完了時点でリポジトリ内で変わっているのは **.java ファイルのみ**(既存テストコードの修正を含む)。これに加え、ツールが生成する凍結テスト資産が専用ディレクトリ `characterization-tests/` に追加される。ビルド定義の恒久変更は「ビルド変更レポート」として人間に引き渡す
- 対象プロジェクトに**既存のテストコードがある場合、ベースラインで全パスしたものを合格基準に含める**(4.1.7 節)
...
|------|------|
| Maven / その他ビルドツール | 非対応 |
| JUnit 3/4 系以外の既存テストフレームワーク(TestNG 等) | 非対応。ベースライン時に検出してエスカレーション |
...
│  JUnit 4.13.2 jar / ECJ(ベースラインコンパイラ用)            │
└───────────────────────────────────────────────────────────────┘
      │                          │
...
- **JDK 調達は Docker 一択**。古い JDK の入手性(JDK 7 は Azul Zulu 等)と、Golden Master が JVM ビルドまで含めた挙動を記録することから、全員・全実行で JDK を完全一致させる。JDK 7 に arm64 ビルドは存在しないため `linux/amd64` 固定(Apple Silicon ではエミュレーション実行)
...
- クラスパスはベースライン時の init script ダンプ(5.3 節)から組み立てる
...
経路は `N₀ → … → N_t`(`7→8→11→17→21` の部分列)。変化を 1 つずつ入れるため、各ステップをサブゲートに分解する。失敗原因の切り分け(根拠ベース修正の前提)がゲート分解の目的である。

```
ゲート 0(最初に 1 回だけ: コンパイラ切替の隔離)
    本体コードを javac(-source N₀ -target N₀)でコンパイル
    → 生成物を JDK N₀ の JVM で実行し凍結テスト全パス
    → ここで出るエラーは「コンパイラ差(ベースラインコンパイラ vs javac)」。
      KB の gate=compiler_diff
    ※ 変わるのは本体コードのコンパイラのみ。凍結テストはベースラインコンパイラで
      コンパイル済みのバイトコードをそのまま使う(4.1.6 節)
    ※ ベースラインコンパイラが元から javac のプロジェクトでは自明に通過(実質スキップ)
...
ゲート 0 を独立させる理由: Java 8 以降は javac でビルドする方針のため、コンパイラ切替はどこかで必ず発生する。バージョンアップと同時に切り替えると「コンパイラ差」と「バージョン非互換」が混ざり、エラーの原因帰属(KB の gate 分類)が成立しなくなる。ゲート 0 のコストは全体コンパイル+テスト 1 周分と小さい。

#### 各ステップの重点領域

通過する区間のみが対象になる(開始・終了バージョンの設定による)。
...
バージョンステップは**プロジェクト単位**の状態である(ファイル単位のバージョン到達管理はしない)。「1 ファイルずつ」は修正の粒度(LLM コンテキストと diff 検証の単位)としてのみ用いる。理由: Java のコンパイル単位はプロジェクトであり、ファイル別の到達バージョンは検証可能な実行状態として存在しない。

## 4. ワークフロー

### 4.1 フェーズ 1: ベースライン構築(Characterization Test 生成)

開始バージョン N₀ の環境上で実施する。

#### 4.1.1 Golden Master 方式

期待値の出所は**実行結果**であり、LLM の推測ではない。

1. **LLM の役割は入力生成のみ**: 対象メソッドの呼び出しコード(引数のバリエーション・境界値・例外を誘発しそうな値)を生成する
2. ハーネスがそれをベースライン JVM(= 本番稼働 JVM のメジャーバージョン。設定項目。通常は N₀)上で実行し、戻り値・例外・必要なら引数オブジェクトの事後状態を観測して記録する
3. 記録値から assert を機械的に生成してテストコードを確定する

テストは **JUnit 4(4.13.2)** で作成する(全対象バージョンを同一テストで通すため)。

#### 4.1.2 呼び出しグラフと構築順序

静的解析でプロジェクトの呼び出しグラフを構築し、**末端ノード(プロジェクト内の他メソッドに依存しないメソッド)から**テストを構築する。上位メソッドのテストは検証済みの下位実装をそのまま使う。

#### 4.1.3 境界コードの扱い: パス単位判定
...
サンドボックスは安全装置としても必須である(static イニシャライザ等が本番 DB 接続・ファイル削除・メール送信を実行しうる)。モック自動生成は行わない(モック応答値は LLM の推測であり、Golden Master 方式で排除した「LLM の推測による期待値」が裏口から戻るため)。

#### 4.1.4 非本質挙動の除外(層 1: 予防)
...
凍結テストは「ソース+コンパイル済み jar(ハッシュ付き)」の組とする。**ベースラインのコンパイラ**(= 元のビルド構成のコンパイラ。パイロットでは ECJ、一般には javac のことも多い)により `-source N₀ -target N₀` で一度だけコンパイルし、以後目標バージョンまで**同一バイトコード**を全 JVM で実行する(N₀ レベルのクラスファイルは全後継 JVM で動く。Java 7 ならバージョン 51.0)。

- 各ステップでの再コンパイルは、コンパイラ差をテスト側の挙動差として混入させるため行わない
- ベースラインコンパイラでコンパイルすることで、ベースライン(本体コード+テスト)全体が元の環境で作られたものになり、ゲート 0 で変わる変化が「本体コードのコンパイラ」の 1 つだけになる
- テスト資産は対象ブランチの専用ディレクトリ(`characterization-tests/`)に隔離し、凍結後のコミットで touch されたら機械チェックで検出する

#### 4.1.7 既存テストの取り込み

対象プロジェクトが自前のテストコードを持つ場合、次の規定で合格基準に組み込む:

- ベースライン環境で全既存テストを実行し、**全パスしたテストのみ**を合格基準に採用する。失敗していたテストは除外リストとして記録し、以後の合否判定に使わない
- 既存テストは**凍結対象ではない**。通常の .java ファイルとして修正対象に含まれ、新バージョンでコンパイル非互換が出れば同じ修正ループで修正する。ただし **assert の意味を変える修正は禁止**とし、validate_diff で灰色フラグ扱いにする(ゲートレビューで重点確認)
- アップグレード中に既存テストが失敗した場合、「本体コードの挙動退行」か「テスト自体が非本質挙動を固定していた」かの判別は人間が行う(エスカレーション/再ベースライン承認の枠組みに乗せる)
- 対象は直接 fork 実行系(JUnitCore)で実行可能な JUnit 3/4 系のみ。TestNG 等の他フレームワークはベースライン時に検出してエスカレーション(スコープ外)

### 4.2 フェーズ 2: アップグレードループ

#### 4.2.1 外側ループ(素の Python)

3.2 節のゲートモデルに従い、ゲート内の作業キュー処理・全体検証・ゲート承認待ちを制御する:

```
1. 現在ゲートの条件でプロジェクト全体をコンパイル(または実行)
2. エラーが出たファイルの一覧を抽出 → 作業キュー
3. キューのファイルを 1 つずつ修正(内側修正ループ、4.2.2 節)
4. キューが空になったら全体コンパイル → 全凍結テスト+採用済み既存テスト実行
5. 全件成功でゲート完了 → ステップ末尾なら人間のゲート承認(4.3.1 節)→ 次ステップへ
```

サブプロジェクト間で修正が連鎖するケース(core の API 置換が web を壊す)は、このループが自然に吸収する(次回の全体コンパイルで新たなエラーファイルがキューに載る)。

#### 4.2.2 内側修正ループ(LangGraph、1 ファイル単位)
...
#### 4.2.3 validate_diff(機械判定主・LLM 従)
...
- 灰色フラグ付きパッチ(4.2.3 節・4.1.7 節)はレビューで重点確認
...
4.1.5 節の人間承認。escalations の 1 類型として扱う。

#### 4.3.4 手動修正と validate_diff

人間のコミットは **validate_diff を通さない**(人間の判断は原則 2 の埒外)。ただしコンパイル・テスト検証は同じゲートを通る。責任は承認した人間にある。

#### 4.3.5 操作インターフェース(CLI ファースト)
...
- `gate`: `compiler_diff` / `runtime` / `compile`(3.2 節のゲート分解に対応)
...
- 成功指標としてファイルあたりトークンコストを計測(7 章)

### 5.3 ビルド構成の扱い: init script オーバーレイ

プロジェクトのビルドファイルは一切編集せず、Gradle の `--init-script` で外部から差し替える:

1. **コンパイラと言語レベル**: ステップ/ゲートごとに `options.release`、toolchain、ベースラインコンパイラ(ECJ 等)→ javac 切替を init script 側で構成
...
3. **クラスパスダンプ**: ベースライン時に init script でカスタムタスクを注入し、ソースセット・コンパイル/テストクラスパスを抽出。以後のテスト実行系(3.1 節)はこれを使う
...
1. ツール起動時に対象リポジトリの指定コミットから**ツール専用ブランチ**(`upgrade/7-to-21` 等)を切る
...
正は SQLite 業務テーブル+Git。スキーマ全文は付録 B。

| テーブル | 役割 |
|---------|------|
| project | バージョン進行の管理単位。開始/目標バージョン・現在ステップ/ゲート・ベースライン構成・実行環境の指紋 |
| files | 対象ファイル台帳。現在ゲート内の作業状態のみ持つ(到達バージョンは持たない)。本体/テストの区分と境界ラベル |
| patches | 修正履歴。全 diff・根拠 KB・灰色フラグ・コミット・承認記録 |
| runs | コンパイル・テスト・Golden Master 記録の実行結果とログ |
| knowledge | KB の実行時インデックス(正本は YAML) |
| llm_calls | LLM 呼び出しの完全記録(監査用) |
| escalations | 人間エスカレーションの記録と解決内容 |
...
| フェーズ | 内容 | 完了条件 |
|---------|------|---------|
| P1 基盤 | Docker イメージ(5 JDK 固定・JUnit 4・JavaParser ヘルパー)、SQLite スキーマ、init script オーバーレイ+クラスパスダンプ、JUnitCore 直接 fork 実行系、CLI 骨格 | サンプル Gradle プロジェクトが全ゲート構成(ECJ 互換レベル 7 → javac 各バージョン)でコンパイルでき、手書きテストが 5 JDK 全てで直接 fork 実行できる |
| P2 テスト生成 | 呼び出しグラフ、境界パス判定+SecurityManager サンドボックス、LLM 入力生成 → Golden Master 記録、非本質挙動除外規則、バイトコード凍結、既存テストの検出・採用判定 | サンプルに対し末端から Characterization Test が自動生成され、保証範囲(パス単位カバレッジ)がレポートされる |
...
- **必達**: 凍結した Characterization Test(同一バイトコード)と、合格基準に採用した既存テストが目標バージョン上で全件パスすること。保証は**カバーされたパスの範囲**に対するものであることを明記する
- **保証範囲率**(第一級の報告値): auto-testable と判定されたパスの行・分岐カバレッジ(JaCoCo)
- 自動完走率(エスカレーションなしで目標バージョンへ到達したファイルの割合)
- validate_diff の reject 率(LLM の過剰修正の監視指標)
- **再ベースライン件数**(層 1 除外規則の品質監視。増えるなら 4.1.4 節の規則を強化する)
...
| 既存テスト自体が非本質挙動を固定している | ベースライン全パスのものだけ採用+失敗時は人間判別(本体退行かテストの非本質固定か)。assert 変更は灰色フラグで重点レビュー |
| レガシーコードの実行が危険な副作用を起こす | SecurityManager サンドボックス+Docker の FS/ネットワーク隔離の二重防壁 |
...
| ベースラインコンパイラ(ECJ 等)と javac のコンパイラ差が Java 非互換と混同される | ゲート 0 で隔離し、KB の gate=compiler_diff として別分類 |
...
| 2 | 開始・終了バージョンは LTS 経路(7→8→11→17→21)の部分列として設定可能 | 「汎用ツール」の位置づけと整合させる。パイロットは 7 開始 |
| 3 | バージョン進行はプロジェクト単位。ファイル単位の到達バージョン管理はしない | Java のコンパイル単位はプロジェクトであり、ファイル別バージョンは検証可能な実行状態として存在しない |
| 4 | Golden Master 方式(LLM は入力生成のみ、期待値は実行結果) | LLM 推測の期待値はリトライの収束先が結局実測値になる。Characterization Test の定義は「現状の挙動の固定」 |
| 5 | 呼び出しグラフを構築し末端からテスト構築 | 上位テストが検証済み下位実装を使える |
| 6 | 境界コードはパス単位判定+SecurityManager サンドボックス | メソッド単位二値ではカバレッジが壊滅。サンドボックスは安全装置としても必須 |
| 7 | ベースラインは元のビルド構成(パイロットは Gradle 9.2+ECJ)、コンパイラ切替後は OpenJDK javac | 対象システムの現構成は維持しつつ、以後は標準ビルドへ |
| 8 | ステップをゲート 0 / A / B に分解 | コンパイラ差・ランタイム差・コンパイル非互換を 1 つずつ顕在化させ、根拠ベース修正を成立させる |
| 9 | 凍結テストはベースラインコンパイラでコンパイル。ゲート 0 で変わるのは本体コードのコンパイラのみ。元から javac のプロジェクトではゲート 0 は自明に通過 | ベースライン全体を元の環境で作り、「変化は 1 つずつ」を徹底する(2026-07-08 ヒアリング) |
| 10 | 既存テストはベースライン全パス分を合格基準に採用。凍結せず修正対象に含める(assert 意味変更は禁止・灰色フラグ) | 保証を厚くしつつ、既存テストの非本質固定リスクは人間判別で吸収(2026-07-08 ヒアリング) |
| 11 | init script オーバーレイ、プロジェクトのビルドファイル不変更 | 汎用性と「いつでも元の構成でビルドできる」安全性。恒久化はビルド変更レポートで人間が実施 |
| 12 | 非本質挙動の 2 層対策+人間承認限定の再ベースライン | 凍結原則の唯一の例外を正式なワークフローとして定義 |
| 13 | ゲート単位の人間一括承認(PR 形式) | 症状→根拠→対処→検証結果をステップの文脈のまとまりでレビューできる |
| 14 | マルチモジュールは全サブプロジェクト一斉 | 実行 JVM は結局 1 つ。段階進行は制約管理コストだけが乗る |
| 15 | KB は YAML 正本+SQLite インデックス、半自動ハーベスト+人間レビュー | KB はツールの資産。レビュー・版管理・共有可能に |
| 16 | validate_diff は JavaParser ヘルパー+機械判定主・LLM 従、SEARCH/REPLACE 形式 | javalang はメンテ停止。LLM 同士の審査は抜け穴になる |
| 17 | モデルティア+temperature 0+llm_calls 全記録+トークン予算 | 監査可能性とコスト暴走防止 |
| 18 | Docker 単一イメージ(5 JDK 固定、linux/amd64) | JDK 7 の入手性と JVM ビルドレベルまでの再現性。arm64 ビルドは存在しない |
| 19 | Git ブランチ+1 パッチ 1 コミット+ゲートタグ、テストはバイトコード凍結 | 巻き戻しは Git に任せる。ゲートレビューが普通の PR になる |
| 20 | LangGraph は内側修正ループのみ、外側は素の Python+SQLite が正 | 正を 2 つにしない。決定的な工程管理にエージェント基盤は不要 |
| 21 | CLI ファースト、独自 UI なし。手動修正は validate_diff 免除 | 既存の仕事場(エディタ・Git・PR)に情報を届ける。人間の判断は原則 2 の埒外 |
...
CREATE TABLE project (
    ...
    start_version     TEXT NOT NULL,        -- 開始 LTS('7' 等)
    target_version    TEXT NOT NULL,        -- 目標 LTS('21' 等)
    baseline_jvm      TEXT NOT NULL,        -- 本番稼働 JVM(記録 JVM)
    baseline_compiler TEXT NOT NULL,        -- 元のビルド構成のコンパイラ('ecj' / 'javac')
